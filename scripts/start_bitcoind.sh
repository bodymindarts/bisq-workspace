...
blocknotify_script=${1?Please provide blocknotify script}
...
debug=1
rpcserialversion=0
blocknotify=$blocknotify_script %s
EOF

echo "Starting bitcoin daemon in regtest mode"
bitcoind -daemon -rpcport=${BITCOIN_RPC_PORT} -conf=${BITCOIN_CONF}

CMD="bitcoin-cli -regtest -datadir=${BITCOIN_DIR} -rpcuser=bisq -rpcpassword=bisq -rpcport=${BITCOIN_RPC_PORT}"
...
${CMD} generatetoaddress 101 ${FAUCET}
